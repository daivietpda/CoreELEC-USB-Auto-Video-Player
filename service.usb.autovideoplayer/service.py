...
class USBAutoPlayerService:
    ...
    def run(self):
        ...
        while not self.monitor.abortRequested():
            try:
                # Check for manual play IPC trigger from addon.py via Window(10000)
                win = xbmcgui.Window(10000)
                trigger = win.getProperty("service.usb.autovideoplayer.trigger")
                if trigger and trigger.startswith("play"):
            ...
            # Wait 0.5s before next cycle (responsive, low-CPU, instant exit on Kodi shutdown)
